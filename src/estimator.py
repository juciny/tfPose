...
class TfPoseEstimator:
    ...
    def get_human_shouder(npimg,humans,imgcopy=False):
        if imgcopy:
            npimg = np.copy(npimg)
        ...
        if centers and centers[1] and centers[2]  and centers[5] :
            human_center = centers[1]
            width = centers[5][0] - centers[2][0]
            human_shoudler = abs(width)

            print("human center:",human_center)
            print("human shoudler:",human_shoudler)
            ...
            # 缩放logo
            resize_width = int(logo_width * human_shoudler / logo_shoudler)
            resize_height = int(logo_height * human_shoudler / logo_shoudler)
            logo = cv2.resize(logo, (resize_width, resize_height), interpolation=cv2.INTER_AREA)

            # 计算logo在frame上的中心
            logo_center_x = int(logo_center_x * human_shoudler / logo_shoudler)
            logo_center_y = int(logo_center_y * human_shoudler / logo_shoudler)
            print("new logo center:", logo_center_x, logo_center_y)
            ...
            #把logo放到合适的位置上，并进行裁剪
            if human_center[0]<logo_center_x:
                d=int(logo_center_x-human_center[0])
                logo = logo[d:resize_width-d, 0:resize_height]
                resize_width -= d*2
                #logo_center_x -= d
            else:
                begin_x = int(human_center[0] - logo_center_x)


            if human_center[1]<logo_center_y:
                d=int(logo_center_y-human_center[1])
                logo = logo[0:resize_width, d:resize_height-d]
                resize_height -= d*2
                #logo_center_y -= d
            else:
                begin_y = int(human_center[1] - logo_center_y)


            print("resize size:",resize_width,resize_height)
            #print("changed logo center:",logo_center_x,logo_center_y)
            print("begin point",begin_x,begin_y)


            if resize_width+begin_x>image_w:
                end_x=image_w
                d=resize_width+begin_x-image_w
                logo=logo[0:resize_width-d,0:resize_height]
            else:
                end_x=resize_width+begin_x

            if resize_height+begin_y>image_h:
                end_y=image_h
                d=resize_height+begin_y-image_h
                logo=logo[0:resize_width,0:resize_height-d]
            else:
                end_y=resize_height+begin_y
            ...
            # 获取frame的通道
            dst_channels = cv2.split(npimg)

            for i in range(3):
                dst_channels[i][begin_x:end_x,begin_y:end_y] = dst_channels[i][begin_x:end_x,begin_y:end_y] * (255.0 - a) / 255
                dst_channels[i][begin_x:end_x,begin_y:end_y] += np.array(src_channels[i] * (a / 255), dtype=np.uint8)
